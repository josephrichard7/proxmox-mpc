...
export class DiagnosticsCollector {
  ...
  private constructor() {
    ...
    this.anonymizationEngine = AnonymizationEngine.getInstance();
  }
  ...
  async generateSnapshot(
    ...
    const snapshot: DiagnosticSnapshot = {
      ...
      workspaceInfo: workspace
        ? await this.getWorkspaceInfo(workspace)
        : undefined,
    };
  ...
  async performHealthChecks(): Promise<HealthStatus[]> {
    ...
    try {
      ...
      if (fs.existsSync(proxmoxDir)) {
        ...
        const hasTerraform = fs.existsSync(
  ...
  /**
   * Anonymize diagnostic snapshot for safe sharing
   */
  async anonymizeSnapshot(
    snapshot: DiagnosticSnapshot,
  ): Promise<AnonymizedData<DiagnosticSnapshot>> {
    return await this.anonymizationEngine.anonymize(
      snapshot,
      DEFAULT_ANONYMIZATION_OPTIONS,
    );
  }
  ...
  /**
   * Generate safe diagnostic snapshot (anonymized by default)
   */
  async generateSafeSnapshot(
    workspace?: string,
    operation?: string,
    error?: any,
  ): Promise<AnonymizedData<DiagnosticSnapshot>> {
    const rawSnapshot = await this.generateSnapshot(
      workspace,
      operation,
      error,
    );
    return await this.anonymizeSnapshot(rawSnapshot);
  }
