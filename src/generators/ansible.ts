...
${siteYaml}`);
    ...
    if (vms.length > 0) {
      await this.generateVMsPlaybook(vms);
    }

    // Generate container-specific playbook
    if (containers.length > 0) {
      await this.generateContainersPlaybook(containers);
    }
  ...
  /**
   * Generate individual VM playbook
   */
  async generateVMPlaybook(vm: VMInfo): Promise<void> {
    const resourceName = this.sanitizeResourceName(vm.name || `vm-${vm.vmid}`);
    
    const vmPlaybook = [
      {
        name: `Configure VM: ${vm.name || vm.vmid}`,
        hosts: 'localhost',
        gather_facts: false,
        vars: {
          vm_config: {
            ...
            node: vm.node,
            cores: vm.cpus || 2,
            memory: vm.maxmem ? Math.floor(vm.maxmem / 1024 / 1024) : 2048,
            storage: 'local-lvm',
          }
        },
        tasks: [
          {
            name: 'Ensure VM configuration is documented',
            debug: {
              msg: [
          ...
          {
            name: 'TODO: Add VM-specific configuration tasks',
            debug: {
              msg: 'This playbook will configure VM {{ vm_config.name }} after deployment'
            }
          }
        ]
      }
    ...
    const playbookPath = path.join(playbooksDir, `${resourceName}.yml`);
    const playbookYaml = yaml.dump(vmPlaybook, { 
      indent: 2, 
      lineWidth: -1,
      noRefs: true 
    });

    await fs.writeFile(playbookPath, `# VM Playbook: ${vm.name || vm.vmid}
# Generated by Proxmox-MPC
# Last updated: ${new Date().toISOString()}
...
${playbookYaml}`);
  ...
  /**
   * Generate individual container playbook  
   */
  async generateContainerPlaybook(container: ContainerInfo): Promise<void> {
    const resourceName = this.sanitizeResourceName(container.name || `ct-${container.vmid}`);
    
    const containerPlaybook = [
      {
        name: `Configure Container: ${container.name || container.vmid}`,
        hosts: 'localhost',
        gather_facts: false,
        vars: {
          container_config: {
        ...
        tasks: [
          {
            name: 'Ensure container configuration is documented',
            debug: {
              msg: [
                'Container Name: {{ container_config.name }}',
                'VMID: {{ container_config.vmid }}',
                'Node: {{ container_config.node }}',
                'Cores: {{ container_config.cores }}',
                'Memory: {{ container_config.memory }}MB'
              ]
            }
          },
          {
            name: 'TODO: Add container-specific configuration tasks',
            debug: {
              msg: 'This playbook will configure container {{ container_config.name }} after deployment'
            }
          }
        ]
      }
    ...
    const playbookPath = path.join(playbooksDir, `${resourceName}.yml`);
    const playbookYaml = yaml.dump(containerPlaybook, { 
      indent: 2, 
      lineWidth: -1,
      noRefs: true 
    });

    await fs.writeFile(playbookPath, `# Container Playbook: ${container.name || container.vmid}
# Generated by Proxmox-MPC
# Last updated: ${new Date().toISOString()}
...
${playbookYaml}`);
  ...
  /**
   * Generate VM-specific playbook (for multiple VMs)
   */
  private async generateVMsPlaybook(vms: VMInfo[]): Promise<void> {
    const vmPlaybook = [
      {
...
${vmYaml}`);
  ...
  /**
   * Generate container-specific playbook (for multiple containers)
   */
  private async generateContainersPlaybook(containers: ContainerInfo[]): Promise<void> {
    const containerPlaybook = [
      {
...
${maintenanceYaml}`);
  ...
  /**
   * Sanitize names for resource names
   */
